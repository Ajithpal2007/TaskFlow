...
import { Download, FileImage, FileText, Send, X, Headphones, MessageSquareReply, ExternalLink } from "lucide-react";
...
export default function ChatRoom({ channelId }: { channelId: string }) {
  ...
  useEffect(() => {
    if (!channelId) return;
    apiClient.get(`/chat/channels/${channelId}`).then((res) => setChannel(res.data.data)).catch(console.error);
  }, [channelId]);
      ...
      <div className="h-14 px-6 border-b flex justify-between items-center bg-background shrink-0 z-20">
        <h3 className="font-bold text-lg flex items-center gap-2">
          {!channel ? (
            ...
            // 🟢 IF DIRECT MESSAGE: Show an @ symbol and the user's name
            <>
              <span className="text-muted-foreground">@</span>
              {/* If your backend sends the 'other' user's name, put it here. Otherwise fallback. */}
              {channel.name || "Direct Message"}
            </>
          ) : (
            // 🟢 IF STANDARD CHANNEL: Show the # symbol and channel name
            <>
              <span className="text-muted-foreground">#</span>
              {channel.name}
            </>
