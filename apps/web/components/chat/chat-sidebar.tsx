...
export function ChatSidebar() {
  ...
  const { mutate: deleteChannel, isPending } = useDeleteChannel(workspaceId);
  ...
  return (
    <>
      <div className="w-64 min-w-[256px] flex-shrink-0 border-r bg-background/50 flex flex-col h-full overflow-hidden">
                  ...
                  {directMessages.map((dm) => {
                    ...
                    return (
                      <Link key={dm.id} href={`/dashboard/${workspaceId}/chat/${dm.id}`} className={`flex items-center gap-3 px-2 py-1.5 text-sm rounded-md transition-colors ${isActive ? "bg-primary/10 text-primary font-medium" : "text-muted-foreground hover:bg-muted"}`}>
                        {otherMember?.image ? (
                          <Image width={36}
                            height={36} src={otherMember.image} alt="" className="h-5 w-5 rounded-full object-cover shrink-0" />
                        ) : (
